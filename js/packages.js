...
/**
 * 生成一个 IC 封装的 Node 配置。
 * @param {string} type   'SOP' | 'LQFP' | 'QFN'
 ...
 *   - af   : PORT(如 "PA0") -> [AF0..AF7] 功能名数组（'-' 表示该 AF 无功能）
 *   - pins : 物理引脚顺序（SOP 封装，Pin1 在左上，逆时针），
 *            entry 为 { label, port }，port=null 表示电源/无复用引脚
 *
 * 右键设备上的 Pad，AF 菜单会按该 PORT 的 AF0–AF7 列表列出可选功能。
 */
...
function buildDevice(deviceName) {
    ...
    const pins = dev.pins.map((p, i) => ({ label: p.label, port: p.port, pin: i + 1 }));
...
function buildCustomDevice(def) {
    const pins = (def.pins || []).map((p, i) => Object.assign({}, p, { pin: i + 1 }));
    const half = Math.ceil(pins.length / 2);
    const left = pins.slice(0, half);          // 左：上→下
    // 模拟接口转换器：右侧不翻转，使 left[i]↔right[i] 在视觉与高亮上成对直通
    // （左=GPIO、右=接口；如 GPIO_SCK ↔ SPI_SCK / GPIO_MOSI ↔ SPI_MOSI）。
    const right = def.converter ? pins.slice(half) : pins.slice(half).reverse();
    const isFlat = pins.length <= 2;
 ...
 *   - bus     : 总线前缀（SPI / I2C / UART / USART / TIM ...，可含数字如 I2C）
 *   - instance: 带编号的实例名（SPI1 / I2C1 ...）
 *   - signal  : 信号名（MOSI / SCL ...）
 * 约定：实例编号为 id 中 “_” 之前最后一段数字；其前者为总线前缀。
 * 无法解析（如 "MCO" / "SWDIO" / "GPIO"，无下划线或前缀无编号）返回 null。
 */
function parseFunctionId(id) {
    ...
    if (!m) return null;
    const bus = head.slice(0, m.index).toUpperCase();
    return {
        bus: bus,
